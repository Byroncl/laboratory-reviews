...
import { ICategoryRepository, ICategory } from '../../interfaces/category.interface';
...
export class CategoryRepository implements ICategoryRepository {
  ...
  async create(category: ICategory): Promise<ICategory> {
    const existingByName = await this.categoryModel.findOne({ name: category.name }).exec();
    if (existingByName) {
      throw new BadRequestException(CATEGORY_MESSAGES.NAME_ALREADY_EXISTS);
    }

    const existingBySlug = await this.categoryModel.findOne({ slug: category.slug }).exec();
    if (existingBySlug) {
      throw new BadRequestException(CATEGORY_MESSAGES.SLUG_ALREADY_EXISTS);
    }

    const entity = new CategoryEntity(category);
    const created = await this.categoryModel.create(this.mapper.toPersistence(entity));
  ...
  async findAll(skip: number, limit: number, filters?: any): Promise<{ items: ICategory[]; total: number }> {
    ...
    return {
      items: items.map(item => this.mapper.toResponse(this.mapper.toDomain(item))),
      total,
    };
  }
  ...
  async findActive(): Promise<ICategory[]> {
    ...
    return categories.map(cat => this.mapper.toResponse(this.mapper.toDomain(cat)));
  }
  ...
  async update(id: string, category: Partial<ICategory>): Promise<ICategory | null> {
    ...
    if (category.name && category.name !== existing.name) {
      const dup = await this.categoryModel.findOne({ name: category.name }).exec();
      if (dup) {
        throw new BadRequestException(CATEGORY_MESSAGES.NAME_ALREADY_EXISTS);
      }
    }
    ...
    if (category.slug && category.slug !== existing.slug) {
      const dup = await this.categoryModel.findOne({ slug: category.slug }).exec();
      if (dup) {
        throw new BadRequestException(CATEGORY_MESSAGES.SLUG_ALREADY_EXISTS);
      }
    }
    ...
    const result = await this.categoryModel.findByIdAndUpdate(id, this.mapper.toPersistence(updated), {
      new: true,
    });
  ...
  async bulkCreate(categories: ICategory[]): Promise<ICategory[]> {
    ...
    const duplicates = await this.categoryModel.find({ slug: { $in: slugs } }).exec();

    if (duplicates.length > 0) {
      const duplicateSlugs = duplicates.map(d => d.slug).join(', ');
      throw new BadRequestException(`Las siguientes categorías ya existen: ${duplicateSlugs}`);
    }
    ...
    return created.map(item => this.mapper.toResponse(this.mapper.toDomain(item)));
  }

  async incrementPostsCount(id: string): Promise<void> {
    await this.categoryModel.findByIdAndUpdate(id, { $inc: { postsCount: 1 } }).exec();
  }

  async decrementPostsCount(id: string): Promise<void> {
    await this.categoryModel.updateOne({ _id: id, postsCount: { $gt: 0 } }, { $inc: { postsCount: -1 } }).exec();
  }
